...
Now I want to swich from the original project to my fork. I have to change the directive SETUPFILE and the path in publish-project.el.
#+begin_src 
#+SETUPFILE: ~/org/aw-org-html-themes/setup/theme-readtheorg-local.setup
#+Options: \n:t
#+Title: live-scripting
...
The publish process does not respect the folder structure. It expects a style folder on the same directory level. The solution must take into account the option to replicate the static website to a server in the internet. 

Write a function that is called by :completion-function. It gets the projectPropertyList as an argument. This list contains the publishingDirectory. The function will call a shell script that uses the publishingDirectory to handle the missing style directories.


#+begin_src 
lubuntu@lubuntu-pc:~/org/live-scripting$ cat foo.txt 
...
;;(defun myFunction ()
(setq projectPropertyList="foobar")
